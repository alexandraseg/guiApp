...
public class DrawTextPanel extends JPanel  {
	...
	ArrayList<DrawTextItem> theString = new ArrayList<DrawTextItem>();
...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
	...
	public JMenuBar getMenuBar() {
		if (menuBar == null) {
			...
			JMenuItem colorize = new JMenuItem("Colorize...");
			colorize.setAccelerator(KeyStroke.getKeyStroke(commandKey + "O"));
			colorize.addActionListener(menuHandler);
			fileMenu.add(colorize);
	...
	private void doMenuCommand(String command) {
		...
		//PART 3 : IMPROVEMENT OF THE PROGRAM!
		else if (command.equals("Colorize...")){
			colorize();
			canvas.repaint();
		}
	...
	private void colorize() {
		...
		for (DrawTextItem item: theString){
			item.setTextColor(new Color (random.nextInt()));
		}
	}
	...
	private void open() {
		...
		try {
			...
			if (scanner.hasNextLine()){
				...
				canvas.setBackground(new Color(colour));
			}
			theString = new ArrayList<DrawTextItem>();
			while (scanner.hasNextLine()){
				String line = scanner.nextLine();
				String[] parts = line.split(",");
				int colour = Integer.parseInt(parts[0]);
				int x = Integer.parseInt(parts[1]);
				int y = Integer.parseInt(parts[2]);
				String text = parts[3];
				DrawTextItem item = new DrawTextItem(text, x, y);
				item.setTextColor(new Color(colour));
				theString.add(item);
	...
	private void save() {
		// TODO Auto-generated method stub
		File file = fileChooser.getOutputFile();
		try {
			PrintWriter writer = new PrintWriter (file);
			writer.println(canvas.getBackground().getRGB());
			writer.flush();
			for(DrawTextItem item : theString) {
				int colour = item.getTextColor().getRGB();
				int x = item.getX();
				int y = item.getY();
				String string = item.getString();
				writer.println(colour+","+x+","+y+","+string);
				writer.flush();
			}
			writer.close();
		} catch (FileNotFoundException e) {
